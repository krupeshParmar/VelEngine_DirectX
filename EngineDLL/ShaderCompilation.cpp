...
using namespace vel;
//using namespace vel::graphics::d3d12::shaders;
...
namespace {
	...
	constexpr const char* shaders_source_path{ "../../VelEngine/Graphics/Direct3D12/Shaders/" };
	...
	Scope<u8[]> pack_compiled_shader(dxc_compiled_shader compiled_shader, bool include_errors_and_disassembly)
	{
		if (compiled_shader.byte_code && compiled_shader.byte_code->GetBufferPointer() && compiled_shader.byte_code->GetBufferSize())
		{
			...
			Scope<u8[]> buffer{ CreateScope<u8[]>(buffer_size) };
...
Scope<u8[]> compile_shader(shader_file_info info, u8* code, u32 code_size, utl::vector<std::wstring>& extra_args, bool include_errors_and_disassembly/*=false*/)
{
...
Scope<u8[]> compile_shader(shader_file_info info, const char* file_path, utl::vector<std::wstring>& extra_args, bool include_errors_and_disassembly/*=false*/)
{
