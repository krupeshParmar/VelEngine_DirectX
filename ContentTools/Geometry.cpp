...
namespace vel::tools
{
	namespace
	{
					...
					if (!is_hard_edge)
					{
						for (u32 k{ j + 1 }; k < num_refs; ++k)
						{
							...
							if (!is_soft_edge)
							{
								...
								//		 possible change in this loop iteration. We assume unit length for n2.
		...
		u64 get_mesh_size(const mesh& m)
		{
			...
			const u64 size{
				su32 + m.name.size() +			// mesh name length and name value
				su32 +							// lod id
				su32 +							// vertex size
		...
		void pack_mesh_data(const mesh& m, u8* const buffer, u64& at)
		{
			...
			memcpy(&buffer[at], &s, su32); at += su32;
			// LOD thresold
			memcpy(&buffer[at], &m.lod_threshold, sizeof(s32)); at += sizeof(s32);
			// vertex data
