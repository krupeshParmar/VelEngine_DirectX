...
namespace vel::tools
{
	namespace {
		...
		primitive_mesh_creator creators_list[]
		{
			create_plane,
			create_cube,
			create_uv_sphere,
		...
		static_assert(_countof(creators_list) == primitive_mesh_type::count);
		...
		mesh create_uv_sphere(const primitive_init_info& info)
		{
			...
			for (u32 j{ 1 }; j <= (theta_count - 1); ++j)
			{
				...
				/*const f32 y{ cos(theta) };
				const f32 r{ sin(theta) };*/
		...
		void create_uv_sphere(scene& scene, const primitive_init_info& info)
		{
			lod_group lod{};
			lod.name = "uv_sphere";
			lod.meshes_list.emplace_back(create_uv_sphere(info));
			scene.lod_groups.emplace_back(lod);
		}
		void create_ico_sphere(scene& scene, const primitive_init_info& info)
		{
