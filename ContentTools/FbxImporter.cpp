...
namespace vel::tools {
		...
		fbx_context::get_lod_group(FbxNode* node)
	{
		...
		if (FbxLODGroup * lod_grp{ node->GetLodGroup() })
		{
			...
			for (s32 i{ 0 }; i < num_nodes; ++i)
			{
				get_mesh(node->GetChild(i), lod.meshes_list);

				if (lod.meshes_list.size() > 1 && lod.meshes_list.size() <= num_lods + 1 &&
					lod.meshes_list.back().lod_threshold < 0.f)
				{
					FbxDistance threshold;
					lod_grp->GetThreshold((u32)lod.meshes_list.size() - 2, threshold);
					lod.meshes_list.back().lod_threshold = threshold.value() * _scene_scale;
				}
			}
