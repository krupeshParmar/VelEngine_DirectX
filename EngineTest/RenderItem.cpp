...
create_render_items()
{
    //assert(std::filesystem::exists("..\\..\\x64\\stripclub.model"));
    assert(std::filesystem::exists("..\\..\\x64\\sword.model"));
    assert(std::filesystem::exists("..\\..\\x64\\maria.model"));
    auto _1 = std::thread{ [] { lab_model_id = load_model("..\\..\\x64\\stripclub_interior2.model"); } };
    auto _2 = std::thread{ [] { fan_model_id = load_model("..\\..\\x64\\sword.model"); } };
    auto _3 = std::thread{ [] { int_model_id = load_model("..\\..\\x64\\maria.model"); } };
    auto _4 = std::thread{ [] { load_shaders(); } };

    lab_entity_id = create_one_game_entity({ -5.f, -1.f, 0.f }, {}, nullptr).get_id();
    fan_entity_id = create_one_game_entity({ 2.f, 1.3f, -6.6f }, {}, "wibbly_wobbly_script").get_id();
    int_entity_id = create_one_game_entity({ 0.f, 1.3f, -6.6f }, {}, "rotator_script").get_id();
