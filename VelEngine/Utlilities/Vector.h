...
namespace vel::utl
{
	...
	public:
		...
		// insert item by copying the value
		constexpr void push_back(const T& value)
		{
			emplace_back(value);
		}

		// insert item by mvoing the value
		...
		constexpr decltype(auto) emplace_back(params&&... p)
		{
			if (_size == _capacity)
			{
				reserve(((_capacity + 1) * 3) >> 1);	// reserve 50% more memory
			}
		...
		// removes the item at the given index in order
		...
		// removes the given item in order
		...
		// removes the item at the given index unordered
		...
		// removes the given item unordered
		...
		// Resize with a given value
		constexpr void resize(u64 new_size, T& value)
		{
