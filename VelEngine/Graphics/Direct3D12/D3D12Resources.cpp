...
namespace vel::graphics::d3d12
{
	...
	bool descriptor_heap::initialize(u32 capacity, bool is_shader_visible)
	{
		...
		for (u32 i{ 0 }; i < capacity; ++i) _free_handles_list[i] = i;
		DEBUG_OP(for (u32 i{ 0 }; i < frame_buffer_count; ++i) assert(_deferred_free_indices_list[i].empty()));
	...
	void descriptor_heap::process_deferred_free(u32 frame_idx)
	{
		...
		utl::vector<u32>& indices{ _deferred_free_indices_list[frame_idx] };
		if (!indices.empty())
		{
			for (auto index : indices)
			{
				--_size;
				_free_handles_list[_size] = index;
			}
			indices.clear();
		}
	}
	...
	descriptor_handle descriptor_heap::allocate()
	{
		...
		const u32 index{ _free_handles_list[_size] };
	...
	void descriptor_heap::free(descriptor_handle& handle)
	{
		...
		_deferred_free_indices_list[frame_idx].push_back(index);
