...
namespace vel::graphics::d3d12::content
{
    namespace {
        ...
        struct {
            utl::vector<vel::content::lod_offset>       lod_offsets;
...
#pragma intrinsic(_BitScanForward)
            ...
            { // Lock materials
                ...
                for (u32 i{ 0 }; i < shader_type::count; ++i)
                {
                    if (flags & (1 << i))
                    {
                        ...
                        vel::content::compiled_shader_ptr shader{ vel::content::get_shader(material.shader_ids()[shader_index], key) };
                        assert(shader);
    ...
    namespace render_item {
                ...
                u32 material_count, const id::id_type *const material_ids)
        {
            ...
            vel::content::get_submesh_gpu_ids(geometry_content_id, material_count, gpu_ids);
        ...
        void get_d3d12_render_item_ids(const frame_info& info, utl::vector<id::id_type>& d3d12_render_item_ids)
        {
            ...
            vel::content::get_lod_offsets(frame_cache.geometry_ids.data(), info.thresholds, count, frame_cache.lod_offsets);
            ...
            for (u32 i{ 0 }; i < count; ++i)
            {
                ...
                const vel::content::lod_offset& lod_offset{ frame_cache.lod_offsets[i] };
