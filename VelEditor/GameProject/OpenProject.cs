...
namespace VelEditor.GameProject
{
    ...
    class OpenProject
    {
        ...
        private static void ReadProjectData()
        {
            ...
            {
                var projects = Serializer.FromFile<ProjectDataList>(_projectDataPath).ProjectList.OrderByDescending(x => x.Date);
                _projects.Clear();
                foreach(var project in projects)
                {
                    if(File.Exists(project.FullPath))
                    {
                        project.Icon = File.ReadAllBytes($@"{project.ProjectPath}\.Vel\Icon.png");
                        project.Screenshot = File.ReadAllBytes($@"{project.ProjectPath}\.Vel\Screenshot.png");
        ...
        private static void WriteProjectData()
        {
            ...
            Serializer.ToFile(new ProjectDataList() { ProjectList = projects }, _projectDataPath);
        }
        ...
        static OpenProject()
        {
            try
            {
                if(!Directory.Exists(_applicationLocalDataPath)) Directory.CreateDirectory(_applicationLocalDataPath);
                _projectDataPath = $@"{_applicationLocalDataPath}ProjectData.xml";
